...
<main class="py-6 px-4 md:px-8">
    ...
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-4">
        ...
        <div class="flex flex-wrap gap-2">
            <!-- SATU TOMBOL AJA -->
            <a href="{{ route('students.create') }}" class="inline-flex items-center px-4 py-2 bg-blue-500 hover:bg-blue-600 text-white text-sm font-medium rounded-md transition duration-200">
                <i class="fas fa-user-plus mr-2"></i>
                <span>Tambah Mahasiswa</span>
            </a>
            <button onclick="openImportModal()" class="inline-flex items-center px-4 py-2 bg-green-600 hover:bg-green-700 text-white text-sm font-medium rounded-md transition duration-200">
        ...
        <div class="flex flex-wrap justify-between items-center px-6 py-4 border-b border-gray-200 gap-4">
            <div class="flex-grow">
                <form action="{{ route('students.index') }}" method="GET" class="flex items-center gap-2">
                    <input type="text" name="search" placeholder="Cari nama, NIM, atau username..." 
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        ...
                        <tr class="hover:bg-gray-50">
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">
                                <code class="bg-blue-50 px-2 py-1 rounded text-xs text-blue-700">{{ $student->nim }}</code>
                            </td>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-600">
                                @if($student->username)
                                    <code class="bg-gray-100 px-2 py-1 rounded text-xs">{{ $student->username }}</code>
                                @else
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-600">
                                <div>
                                    <span class="font-medium">{{ $student->studentClass->name ?? '-' }}</span>
                                    @if($student->year)
            ...
            <div class="mt-4">
                <div class="mb-4">
                    ...
                    <input type="file" 
                           ...
                           accept=".xlsx,.xls,.csv" 
                           required>
                    <p class="mt-1 text-sm text-gray-500">Format: NIM, Nama, Email, Username, Password</p>
                </div>
            </div>
