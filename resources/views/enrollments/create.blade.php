            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <select name="status" id="status" 
                        ...
                        <option value="enrolled" {{ old('status', 'enrolled') === 'enrolled' ? 'selected' : '' }}>Terdaftar</option>
                ...
                <div class="flex items-start">
                    ...
                    <div class="text-sm text-blue-800">
                        ...
                        <ul class="list-disc list-inside mt-1 space-y-1">
                            ...
                            <li>Satu mahasiswa tidak dapat didaftarkan dua kali pada mata kuliah yang sama di period yang sama</li>
                        </ul>
                    </div>
                </div>
            </div>
        ...
        <div class="p-6">
            <form action="{{ route('enrollments.bulk') }}" method="POST" id="bulkForm">
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
                    <div>
                        <label for="bulk_period_id" class="block text-sm font-medium text-gray-700 mb-2">Period</label>
                        <select name="period_id" id="bulk_period_id" class="w-full border border-gray-300 rounded-md px-3 py-2" required>
                            <option value="">Pilih Period</option>
                            @foreach($periods as $period)
                    ...
                    <div>
                        <label for="bulk_course_id" class="block text-sm font-medium text-gray-700 mb-2">Mata Kuliah</label>
                        <select name="course_id" id="bulk_course_id" class="w-full border border-gray-300 rounded-md px-3 py-2" required>
                            <option value="">Pilih Mata Kuliah</option>
                            @foreach($courses as $course)
                    ...
                    <div class="border border-gray-300 rounded-md p-3 max-h-48 overflow-y-auto">
                        @foreach($students as $student)
                            <label class="flex items-center mb-2">
                                ...
                                <span class="text-sm">{{ $student->name }} - {{ $student->nim }}</span>
                            </label>
                        @endforeach
                    </div>
                </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    periodSelect.addEventListener('change', function() {
        ...
        if (this.value) {
            if (isEnrollmentOpen) {
                periodStatus.innerHTML = '<i class="fas fa-check-circle text-green-600 mr-1"></i>Masa pendaftaran masih terbuka';
                periodStatus.className = 'text-xs text-green-600 mt-1';
            } else {
                periodStatus.innerHTML = '<i class="fas fa-exclamation-triangle text-yellow-600 mr-1"></i>Masa pendaftaran sudah tutup';
    ...
    selectAllBtn.addEventListener('click', function() {
        ...
        this.innerHTML = allChecked ? '<i class="fas fa-check-square mr-1"></i>Pilih Semua' : '<i class="fas fa-square mr-1"></i>Batal Pilih Semua';
    });
